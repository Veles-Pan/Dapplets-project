...
function fetchApi(_a) {
  var search = _a.search,
      ...
      theme = _a.theme,
      order = _a.order,
      isScrolled = _a.isScrolled,
      changeResultLoading = _a.changeResultLoading,
      filterSearchResult = _a.filterSearchResult,
      createListItems = _a.createListItems,
      setIsScrolled = _a.setIsScrolled,
      stopLoader = _a.stopLoader,
      items = _a.items;
  axios_1['default'].get('' + API_URL + DAPPLETS, {
    params: {
      filter: search ? JSON.stringify([{
        property: 'title',
        value: search
      }]) : undefined,
      start: start,
      limit: LIMIT,
      sort: order && theme ? JSON.stringify([{
        property: theme,
        direction: order
      }]) : undefined
    }
