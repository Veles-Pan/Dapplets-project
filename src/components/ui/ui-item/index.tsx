...
import installed from '../../../assets/global/installed.svg'
...
const Item = (props: IProps) => {
    const storageKey = `Installed-${props.item.id}`

    const [isExtraOpened, changeOpening] = useState<boolean>(false)
    const [isInstalled, changeInstallation] = useState<boolean>(
        localStorage.getItem(storageKey) == 'false' ? false : true
    )
    ...
    return (
        ...
        >
            ...
            <div
                className='item__install'
                style={{
                    background: isInstalled ? '#0085ff' : '#CCD4DB',
                }}
                onClick={event => {
                    event.stopPropagation()
                    changeInstallation(prevState => !prevState)
                    localStorage.setItem(storageKey, (!isInstalled).toString())
                }}
            >
                <p className='install'>
                    {isInstalled ? 'Install' : 'Installed'}
                </p>
                <img
                    alt='install'
                    src={isInstalled ? install : installed}
                    className='install_icon'
                />
                <p className='uninstall'>
                    {isInstalled ? 'Install' : 'Uninstall'}
                </p>
                <img
                    alt='uninstall'
                    src={isInstalled ? install : uninstall}
